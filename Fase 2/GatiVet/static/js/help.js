...
async function loadCases() {
    try {
        ...
        cases.forEach(caseItem => {
            ...
            // Establecer ancho fijo para todas las cards
            caseCard.style.width = '300px';  // Ancho fijo para las cards
            caseCard.style.display = 'flex';
            caseCard.style.flexDirection = 'column';
            caseCard.style.marginBottom = '10px';
            caseCard.style.transition = 'duration-300';
            caseCard.className = "mx-auto max-w-xs rounded-lg overflow-hidden shadow-xl transform-gpu hover:scale-105 hover:bg-[#18beaa] hover:bg-opacity-50 hover:text-white flex flex-col mb-10";
...
async function fetchFundaciones() {
    try {
        ...
        fundaciones.forEach(fundacion => {
            ...
            card.className = "mx-auto transition duration-300 rounded-lg overflow-hidden shadow-xl transform-gpu hover:scale-105 hover:bg-[#18beaa] hover:bg-opacity-50 hover:text-white flex flex-col mb-10";
            
            // Establecer propiedades con JavaScript
            card.style.width = '300px';  // Ancho fijo para las cards
            ...
            fundacionesContainer.appendChild(card);
        });
    } catch (error) {
